...
main (int argc, char *argv[])
{
    ...
    src_x = pixman_fixed_to_int (-xmin);
    src_y = pixman_fixed_to_int (-ymin);

    create_image (xmax - xmin + 64, ymax - ymin, src_format, filter, &t, &src, &src_image);
    if (mask_format)
    {
        create_image (xmax - xmin + 64, ymax - ymin, mask_format, filter, &t, &mask, &mask_image);
